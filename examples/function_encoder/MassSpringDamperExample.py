...
def springmassdamper(state, zeta, omega_n):
    ...
    x2 = state[..., 1]  # velocity
    dx1 = x2
    dx2 = -omega_n ** 2 * x1 - 2 * zeta * omega_n * x2
...
def create_dataset(device='cpu', generate_trajectory = False):
    ...
    zeta_range = torch.tensor([0.0, 1.0], device=device)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        example_xs, example_ys, query_xs, query_ys, zetas, omega_ns = test_dataset["example_xs"], test_dataset["example_ys"], test_dataset["query_xs"], test_dataset["query_ys"], test_dataset["zetas"], test_dataset["omega_ns"]
